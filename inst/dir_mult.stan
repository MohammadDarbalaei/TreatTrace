...
data {
  ...
  int<lower=1> categ_local[C];        // Category assignment for each observation per test
...
parameters {
  ...
  vector<lower=0>[C] S;               // Precision parameter for the Dirichlet-Multinomial
...
generated quantities {
  ...
  for (j in 1:n) {
    y_rep[j] = dirichlet_multinomial_rng(S .* theta[j], sum(y[j]));  // Generate replicated data
    log_lik[j] = dirichlet_multinomial_lpmf(y[j] | S .* theta[j]);   // Compute log-likelihood
  }
}
